...
class MaxHeightContextComponent {
  testElements = vi.fn().mockReturnValue(['#something']);

  somethingHeight = vi.fn().mockReturnValue(22);
  differentHeight = vi.fn().mockReturnValue(0);
}
...
describe('MaxHeightContentContainerComponent', () => {
  ...
  let mockMaxContainerHeightService: MockedObject<MaxContainerHeightService>;
  ...
  beforeEach(() => {
    ...
    mockMaxContainerHeightService = ngMocks.get(MaxContainerHeightService) as MockedObject<MaxContainerHeightService>;
    mockMaxContainerHeightService.calculateMaxContainerHeight = vi.fn().mockReturnValue('calc(100vh - 50px)');
  ...
  it('should subscribe to resizeTrigger and call refreshMaxClientHeight after debounce', async () => {
    ...
    await new Promise(resolve => setTimeout(resolve, 15)); // wait longer than debounce (5ms)
  ...
  it('should have the correct elementSelectorsToSubtract value after change', () => {
    ...
    component.testElements.mockReturnValue(['#something', '#different']);
    fixture.detectChanges();
  ...
  it('should clean up subscriptions in ngOnDestroy', () => {
    const unsubscriberSpy = vi.spyOn((maxHeightContainerComponent as any).unsubscriber, 'next');
    const completeSpy = vi.spyOn((maxHeightContainerComponent as any).unsubscriber, 'complete');
  ...
  it('should update containerHeight signal in refreshMaxClientHeight', () => {
    ...
    const changeDetectorRefSpy = vi.spyOn((maxHeightContainerComponent as any).changeDetectorRef, 'detectChanges');

    mockMaxContainerHeightService.calculateMaxContainerHeight.mockReturnValue('calc(100vh - 75px)');
  ...
  it('should create ResizeObserver and trigger resizeTrigger', () => {
    ...
    component.testElements.mockReturnValue(['#something']);
    const resizeTriggerSpy = vi.spyOn((maxHeightContainerComponent as any).resizeTrigger, 'next');
