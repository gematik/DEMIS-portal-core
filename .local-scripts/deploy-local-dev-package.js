...
const { PACKAGE_NAME, validateTargetProject, checkPortalCoreDependency, runNpmCommand, clearCaches } = require('./package-utils');
...
function findTgzFile() {
  ...
  const tgzFiles = files.filter(file => file.match(/^gematik-demis-portal-core-library-.*\.tgz$/));
...
function main() {
  ...
  if (args.length === 0) {
    console.error('❌ Usage: node install-local-package.js <target-project-path>');
    console.error('Example: node install-local-package.js ../portal-shell');
    process.exit(1);
  }

  const targetPath = path.resolve(args[0]);

  console.log('Portal-Core local installation started');
  ...
  try {
    ...
    // 3. Check Portal-Core dependency
    console.log('\nChecking Portal-Core dependency...');
    const hasPortalCore = checkPortalCoreDependency(packageJson);
